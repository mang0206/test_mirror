    ...
    if sex == '남' :
        if age >= int(nutrients_M.columns[-2][2:4]) :
            ...
            for i in range(len(nutrients_M)-2):
                result.append(round(activity*(int(n_mean[i] + Z*int(n_std[i]))),2))
            return result
        for i in range(9,16,2) :
            if age >= int(nutrients_M.columns[i][:2]) and age <= int(nutrients_M.columns[i][3:5]) :
        ...
        if age >= int(nutrients_W.columns[-2][2:4]) :
            ...
            return result
        for i in range(9,16,2) :
            if age >= int(nutrients_W.columns[i][:2]) and age <= int(nutrients_W.columns[i][3:5]) :
